defmodule Shopify.AdminAPI.Resource do
  ...
  def prepare(resource = %__MODULE__{}) do
    Map.new(method: validate_method(resource.method),
            ...
            body: encode_body(resource.body, resource.name))
  end
  ...
  defp encode_body(body, name) when is_map(body) or is_list(body),
    do: Poison.encode!(%{name => Map.new(body)})
  defp encode_body(body, _) when is_binary(body), do: body
